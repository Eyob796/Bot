/**
 * Belaynish Telegram Bot (Termux Polling)
 ...
 *
 * Usage: set your keys in .env, then run `node index.js` (or `npm start`)
 */
...
async function saveMemory(chatId, history) {
  ...
  if (usingRedis && redis) {
    try {
      // Upstash ioredis supports EX param on set
...
async function callHfApi(prompt, modelUrl = process.env.HF_URL) {
  ...
  const resp = await axios.post(modelUrl, { inputs: prompt }, { headers: { Authorization: `Bearer ${process.env.HUGGINGFACE_API_KEY}` }, timeout: 120000 });
...
async function callReplicateCreate(versionOrModel, input) {
  ...
  const resp = await axios.post(url, { version: versionOrModel, input }, { headers: { Authorization: `Token ${process.env.REPLICATE_API_KEY}`, 'Content-Type': 'application/json' }, timeout: 600000 });
  return resp.data;
}
async function replicateGet(predictionId) {
  const url = `https://api.replicate.com/v1/predictions/${predictionId}`;
  const resp = await axios.get(url, { headers: { Authorization: `Token ${process.env.REPLICATE_API_KEY}` }, timeout: 600000 });
  return resp.data;
}
    ...
    try {
      ...
      if (!progressMsg) {
        ...
        try {
          ...
          try { await ctx.reply(text); } catch (_e) {}
        }
      }
      if (typeof percent === 'number') lastPercent = percent;
    } catch (e) {
      console.warn('onProgress error', e?.message || e);
    }
  };
}
...
async function callRunway(endpoint, model, input) {
  ...
  const r = await axios.post(url, { model, input }, { headers: { Authorization: `Bearer ${process.env.RUNWAY_API_KEY}`, 'Content-Type': 'application/json' }, timeout: 600000 });
  return r.data;
}
...
async function callStabilityImage(prompt, opts = {}) {
  ...
  const r = await axios.post(url, payload, { headers: { Authorization: `Bearer ${process.env.STABILITY_KEY}`, 'Content-Type': 'application/json' }, responseType: 'arraybuffer', timeout: 180000 });
  return r.data;
}
...
async function elevenTTS(text) {
  ...
  const resp = await axios.post(url, { text }, { headers: { 'xi-api-key': process.env.ELEVENLABS_API_KEY, 'Content-Type': 'application/json' }, responseType: 'arraybuffer', timeout: 120000 });
...
bot.command('ai', async (ctx) => {
  ...
  async function withTyping(fn) {
    const typingInterval = setInterval(() => {
      try { ctx.sendChatAction('typing'); } catch (e) {}
    }, 2500);
    try { return await fn(); } finally { clearInterval(typingInterval); }
  }

  try {
    if (mode === 'help') {
      const help = ` /ai <mode> <input>\nChat: /ai chat [model] <prompt> -> models: llama2, mistral, flan_t5, falcon (default llama2)\nSearch: /ai wiki <topic>\n/ai duck <query>\nTranslate: /ai translate [lang] <text> -> default 'en'\nMedia: /ai media <mode> <input> modes: t2i t2v i2v v2v upscale act (Runway) flux fixface caption burncaption recon3d (Replicate)\nTTS: /ai tts <text>\nReplicate direct: /ai replicate <ENV_VAR_NAME> <prompt>\nAdmin: /ai post <@channel_or_channel_username> <message> (admin only)\n/ai clear_memory <chatId> (admin only)\n/ai export_memory <chatId> (admin only)\nType /ai help for this message.`;
    ...
    if (mode === 'chat') {
      ...
      if (process.env.HF_SPACE_URL) {
        try {
          ...
          console.warn('HF Space error:', e.message);
        }
      }
      if (!answer && process.env.HUGGINGFACE_API_KEY) {
        ...
        if (hfModelUrl) {
          try {
            ...
            console.warn('HF API error:', e.message);
          }
        }
      }
      if (!answer && process.env.REPLICATE_API_KEY) {
        try {
          ...
          console.warn('Replicate chat error:', e.message);
        }
      }
      if (!answer) {
    ...
    if (mode === 'tts') {
      ...
      if (process.env.ELEVENLABS_API_KEY && process.env.ELEVENLABS_VOICE_ID) {
        try {
          ...
          // ctx.replyWithVoice accepts a buffer as { source: Buffer }
      ...
      if (process.env.REPLICATE_API_KEY && process.env.REPLICATE_TTS_MODEL) {
        try {
          ...
          console.warn('Replicate TTS error:', e.message);
        }
      }
...
(async () => {
  try {
    console.log('Starting Belaynish bot (polling)...');
